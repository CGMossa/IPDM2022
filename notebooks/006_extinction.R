...
farm_model <- function(n_cows = 100, 
  ...
  for (t in times)
  {
    ...
    is_replaced <- which(past_farm$age >= past_farm$replacement_age);
    ...
    # disease
    susceptible <- which(past_farm$infected == 0)
    total_infected <- sum(past_farm$infected == 1)
    ...
    NewInf <- susceptible[rbinom(length(susceptible), 1, prob = infect_probability) == 1]
    if (length(NewInf) > 0) {
      farm$infected[NewInf] <- 1
    }
    
    # recovery
    pot_recovered <- which(past_farm$infected == 1)
  ...
  list(age_collect = age_collect[c(seq_len(last_t), last_t)],
       ...
       disease_collect = inf_collect[c(seq_len(last_t), last_t),, drop = FALSE] %>% as_tibble())
}
farm_model(
  end_time = 1 * 365,
  ProbInfection = 0.10,
